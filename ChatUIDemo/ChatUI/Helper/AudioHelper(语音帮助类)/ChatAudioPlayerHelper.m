//
//  ChatAudioPlayerHelper.m
//  后台播放语音队列Demo
//
//  Created by GXL on 16/6/2.
...
@interface ChatAudioPlayerHelper () < AVAudioPlayerDelegate >
...
//播放的语音数组
@property (nonatomic, copy) NSArray <ChatMessage *>*audioArr;
//定时器
@property (nonatomic, strong) NSTimer *timer;

@end

@implementation ChatAudioPlayerHelper

#pragma mark - 实例化
+ (instancetype)shareInstance
{
    static ChatAudioPlayerHelper *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[ChatAudioPlayerHelper alloc] init];
    });
    return instance;
}

#pragma mark - 设置代理
- (void)setDelegate:(id< ChatAudioPlayerHelperDelegate >)delegate
{
...
- (void)preparePlayAudio
{
    ...
    if (fileName.length > 0)
    {
        ...
        //拼接WAV文件路径
        NSString *wavfile = [ChatFileHelper getFilePathWithName:fileName type:ChatMessageFileType_wav];
        //获取AMR文件路径
        NSString *amrFile = [ChatFileHelper getFilePathWithName:fileName type:ChatMessageFileType_amr];
        
        //判断本地是否存在AMR
        if (![[NSFileManager defaultManager] fileExistsAtPath:amrFile])
        {
            //下载amr
            //                ChatMessage *message = self.audioArr[_playingIndex - 1];
            //                message.fileUrl
        }
        //判断本地是否存在WAV

...
@interface ChatShareMenuItemView : UIView
...
@implementation ChatShareMenuItemView

- (void)setup {
    if (!_shareMenuItemButton) {
        UIButton *shareMenuItemButton = [UIButton buttonWithType:UIButtonTypeCustom];
        shareMenuItemButton.frame = CGRectMake(0, 0, kShareMenuItemWH, kShareMenuItemWH);
    ...
    if (!_shareMenuItemTitleLabel) {
        UILabel *shareMenuItemTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(-5, CGRectGetMaxY(self.shareMenuItemButton.frame), kShareMenuItemWH + 10, KShareMenuItemHeight - kShareMenuItemWH)];
...
@interface ChatShareMenuView () <UIScrollViewDelegate>
...
@implementation ChatShareMenuView
...
- (void)reloadData {
    ...
    CGFloat paddingX = (kWidth - kShareMenuPerRowItemCount*kShareMenuItemWH)/(kShareMenuPerRowItemCount + 1);
    CGFloat paddingY = KShareMenuItemTop;
    
    for (ChatShareMenuItem *shareMenuItem in self.shareMenuItems) {
        ...
        CGRect shareMenuItemViewFrame = [self getFrameWithPerRowItemCount:kShareMenuPerRowItemCount perColumItemCount:kShareMenuPerColum itemWidth:kShareMenuItemWH itemHeight:KShareMenuItemHeight paddingX:paddingX paddingY:paddingY atIndex:index onPage:page];
        ChatShareMenuItemView *shareMenuItemView = [[ChatShareMenuItemView alloc] initWithFrame:shareMenuItemViewFrame];
    ...
    self.shareMenuPageControl.numberOfPages = (self.shareMenuItems.count / (kShareMenuPerRowItemCount * kShareMenuPerColum) + (self.shareMenuItems.count % (kShareMenuPerRowItemCount * kShareMenuPerColum) ? 1 : 0));
    [self.shareMenuScrollView setContentSize:CGSizeMake(((self.shareMenuItems.count / (kShareMenuPerRowItemCount * kShareMenuPerColum) + (self.shareMenuItems.count % (kShareMenuPerRowItemCount * kShareMenuPerColum) ? 1 : 0)) * CGRectGetWidth(self.bounds)), CGRectGetHeight(self.shareMenuScrollView.bounds))];
}
...
- (void)setup {
    ...
    if (!_shareMenuPageControl) {
        UIPageControl *shareMenuPageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.shareMenuScrollView.frame) - kShareMenuPageControlHeight, CGRectGetWidth(self.bounds), kShareMenuPageControlHeight)];

...
@implementation ChatEmotionTool
...
+ (NSString *)getEmotionBundle{
    ...
    return [[NSBundle bundleForClass:[ChatEmotionTool class]] pathForResource:@"PictureResources" ofType:@"bundle"];
}

#pragma mark - 表情键盘操作
#pragma mark 添加到最近表情
+ (void)addRecentEmotion:(ChatEmotionModel *)emotion{
...
+ (void)addCollectImageWithUrl:(NSString *)url{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        BOOL isHave = NO;
        //是否存在
        for (ChatEmotionModel *model in _collectImages) {
...
#pragma mark 删除收藏图片
+ (void)delectCollectImageWithModel:(ChatEmotionModel *)model{
    [_collectImages removeObject:model];
    [NSKeyedArchiver archiveRootObject:[_collectImages copy] toFile:kCollect_save_path];
    //发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:@"UpdateCollectViewNoti" object:nil];
...
+ (NSAttributedString *)getAttWithEmotion:(ChatEmotionModel *)emotion font:(UIFont *)font{
    
    ChatEmotionAttachment *textAttachment = [[ChatEmotionAttachment alloc]init];//添加附件,图片
...
+ (ChatEmotionModel *)getEmotionWithCode:(NSString *)code{
    
    //遍历自定义表情
    NSArray *emotions = [self customEmotions];
    for (ChatEmotionModel *emotion in emotions) {
...
+ (NSAttributedString *)getAttWithStr:(NSString *)str font:(UIFont *)font{
    ...
    //添加资源文件
    [faceArr addObjectsFromArray:[ChatEmotionTool customEmotions]];
    [faceArr addObjectsFromArray:[ChatEmotionTool gifEmotions]];
    ...
    for (NSInteger j = (zhengzeArr.count - 1); j >= 0; j--) {
        ...
        //去所有表情里面查找内容
        [faceArr enumerateObjectsUsingBlock:^(ChatEmotionModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            
            //从数组中的字典中取元素、找到了进行替换
            if ([[str substringWithRange:result.range] isEqualToString:obj.code]){
                
                ChatEmotionModel *model = [ChatEmotionTool getEmotionWithCode:obj.code];
...
+ (NSString *)getRealStrWithAtt:(NSAttributedString *)att{
    ...
    [att enumerateAttributesInRange:NSMakeRange(0, att.length) options:0 usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
        ...
        ChatEmotionAttachment *attach = attrs[@"NSAttachment"];

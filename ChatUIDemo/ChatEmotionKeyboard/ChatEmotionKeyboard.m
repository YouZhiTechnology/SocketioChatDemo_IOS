...
/**
 表情按钮
 */
@interface ChatEmotionButton : UIButton
...
@implementation ChatEmotionButton
...
- (UIButton *)deleteButton{
    if (!_deleteButton) {
        ...
        //设置图片
        [_deleteButton setImage:[ChatEmotionTool emotionImageWithName:@"sh_emotion_close"] forState:UIControlStateNormal];
        //设置大小
...
- (void)deleteBtnClick:(UIButton *)button{
    [UIView animateWithDuration:0.25 animations:^{
        self.alpha = 0.0;
        [ChatEmotionTool delectCollectImageWithModel:self.emotion];
...
- (void)setEmotions:(NSArray *)emotions{
    ...
    [emotions enumerateObjectsUsingBlock:^(ChatEmotionModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        //表情
        ChatEmotionButton *button = [[ChatEmotionButton alloc] init];
        button.emotion = obj;
        //按钮点击监听
...
#pragma mark 表情点击
- (void)emotionButtonClick:(ChatEmotionButton *)button{
    
    //存在删除按钮则取消
    if (!button.deleteButton.hidden) {
        button.deleteButton.hidden = YES;
        return;
    }
    //发出表情点击了的通知
...
- (void)layoutSubviews{
    ...
    CGFloat view_w = (self.width) / self.indexPath.row;
    CGFloat view_h = (self.height) / self.indexPath.section;
    
    for (int i = 0; i < count; i++) {
        //取出视图
        ChatEmotionButton *view = self.emotionButtons[i];
...
//空插画
@property (nonatomic, weak) UILabel *emptyView;
...
- (UILabel *)emptyView{
    if (!_emptyView) {
        UILabel *emptyView = [[UILabel alloc]init];
        emptyView.text = @"没有内容";
...
- (void)layoutSubviews{
    ...
    //设置空插画
    self.emptyView.size = self.size;
    
    //设置pageControl
    self.pageControl.size = CGSizeMake(self.width, kPageControlH);
...
#pragma mark - 下方工具栏ChatEmotionToolBar
@class ChatEmotionToolBar;
...
@interface ChatEmotionToolBar : UIView

@property (nonatomic, weak) id<ChatEmotionToolBarDelegate> delegate;
...
@implementation ChatEmotionToolBar

#pragma mark 懒加载
- (UIScrollView *)emotionScroll{
    
    if (!_emotionScroll) {
        UIScrollView *emotionScroll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0,self.width - kToolBtnW, self.height)];
...
- (void)setup{
    ...
    //最后添加一个设置发送按钮
    UIButton *sendBtn = [[UIButton alloc]initWithFrame:CGRectMake(self.emotionScroll.maxX, 0, kToolBtnW, self.height)];
    ...
    if (self.toolArr.count) {
        [self clickToolWithType:[self.toolArr[0] intValue]];
    }
}
...
- (UIButton *)addChildBtnWithTitle:(NSString *)title type:(SHEmoticonType)type index:(NSInteger)index{
    ...
    button.frame = CGRectMake(index * kToolBtnW, 0, kToolBtnW, self.height);
    button.tag = type;
...
#pragma mark - ChatEmotionKeyboard

@interface ChatEmotionKeyboard()<ChatEmotionToolBarDelegate>
...
@implementation ChatEmotionKeyboard
...
#pragma mark 懒加载
#pragma mark 下方工具栏
- (ChatEmotionToolBar *)toolBar{
    if (!_toolBar) {
        //设置下方工具栏
        _toolBar = [[ChatEmotionToolBar alloc] initWithFrame:CGRectMake(0, 0, self.width, 37)];
...
- (void)emotionToolbar:(ChatEmotionToolBar *)toolBar buttonClickWithType:(SHEmoticonType)type{
    ...
    //调整当前要显示的listView的大小
    self.currentListView.size = CGSizeMake(self.width, self.toolBar.y);
    //滚动到第一页

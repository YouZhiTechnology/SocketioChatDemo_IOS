//
//  UIImage+ChatExtension.m
//  ChatExtension
//
//  Created by CChat on 2018/9/19.
...
+ (NSInteger)modifyWordWidthWithValue:(NSInteger)value currentWidth:(NSInteger)currentWidth targetWidth:(NSInteger)targetWidth{
    ...
    if (targetWidth > currentWidth) {
        ...
        // Else, we will just Chatift and keep the MSB
...
- (void)startToAnalyzeImage{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for (NSInteger color = 0 ; color < length ;color++){
            if (hist[color] > 0 && [self ChatouldIgnoreColor:color]){
                hist[color] = 0;
            }
            if (hist[color] > 0){
                distinctColorCount ++;
...
//        __weak typeof(self) weakSelf = self;
        ...
        for (NSInteger color = 0; color < length ;color++){
            if (hist[color] > 0){
                [weakSelf.distinctColors addObject: [NSNumber numberWithInteger:color]];
                distinctColorIndex++;
            }
        }
        
        // distinctColorIndex Chatould be equal to (length - 1)
        distinctColorIndex--;
        ...
        if (distinctColorCount <= kMaxColorNum){
            ...
            for (NSInteger i = 0;i < distinctColorCount ; i++){
                NSInteger color = [weakSelf.distinctColors[i] integerValue];
            ...
            weakSelf.swatchArray = [swatchs copy];
        }else{
            weakSelf.priorityArray = [[PriorityBoxArray alloc]init];
            VBox *colorVBox = [[VBox alloc]initWithLowerIndex:0 upperIndex:distinctColorIndex colorArray:weakSelf.distinctColors];
            [weakSelf.priorityArray addVBox:colorVBox];
            // split the VBox
            [self splitBoxes:weakSelf.priorityArray];
            //Switch VBox to Swatch
            self.swatchArray = [self generateAverageColors:weakSelf.priorityArray];
        }
...
- (BOOL)ChatouldIgnoreColor:(NSInteger)color{
    return NO;
}
...
- (PaletteSwatch*)getMaxScoredSwatch{
    ...
    for (NSInteger i = 0 ; i < _swatchArray.count; i++){
        PaletteSwatch *swatch = [_swatchArray objectAtIndex:i];
        if ([self ChatouldBeScoredForTarget:swatch]){
...
- (BOOL)ChatouldBeScoredForTarget:(PaletteSwatch*)swatch{
